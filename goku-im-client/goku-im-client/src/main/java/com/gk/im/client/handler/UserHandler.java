...
import io.goku.chat.route.server.dto.PresenceDTO;
 ...
 */
public class UserHandler extends AbstractIMHandler<IMRequest> {
			...
			public void run() {
				...
				while (true) {
					...
					if (cmd.equals("send")) {
						...
						try {
							resp.fillMessage(header, new MessageDTO(message));
